...
function BookNow() {
  ...
  const getBus = async () => {
    try {
      ...
      const response = await axiosInstance.post(`${process.env.REACT_APP_BACKEND_URL}/api/buses/get-bus-by-id`, {
        _id: params.id
      });
  ...
  const bookNow =
    async ({transactionId}) => {
      try {
        dispatch(ShowLoading());
        const response = await axiosInstance.post(`${process.env.REACT_APP_BACKEND_URL}/api/bookings/book-seat`, {
          bus: bus._id,
          seats: selectedSeats,
          transactionId,
          userId: user
  ...
  const onToken = async (token) => {
    try {
      dispatch(ShowLoading());
      const response = await axiosInstance.post(`${process.env.REACT_APP_BACKEND_URL}/api/bookings/make-payment`, {
        token,
        ...
        <Row className='mt-3' gutter={[30,30]}>
          <Col lg={12} xs={24} sm={24}>
            ...
            <div className='flex flex-col gap-2'>
              <p className='text-md'>Journey Date:{moment(bus.journeyDate).format("DD-MM-YYYY")}</p>
