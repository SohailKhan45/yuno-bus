...
function BusForm( {
    ...
    const onFinish = async(values)=>{
        try{
            ...
            let response = null;
            if(type==='add'){
                response = await axiosInstance.post(`${process.env.REACT_APP_BACKEND_URL}/api/buses/add-bus`,{...values,
                    }
                   // journeyDate : moment(values.journeyDate).format("dd-mm-yyyy"),
                );
            }else{
                response = await axiosInstance.post(`${process.env.REACT_APP_BACKEND_URL}/api/buses/update-bus`,{
                    ...values,
                    _id: selectedBus._id,
                });
            }
